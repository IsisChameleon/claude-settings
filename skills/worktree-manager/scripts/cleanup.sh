...
#
# Usage: ./cleanup.sh <project> <branch> [--delete-branch] [--clean-claude]
#        ./cleanup.sh --merged [--delete-branch] [--clean-claude]
#
# Examples:
...
#   ./cleanup.sh obsidian-ai-agent feature/auth --clean-claude     # Also remove Claude Code project data
#   ./cleanup.sh --merged                                          # Cleanup all merged worktrees
#   ./cleanup.sh --merged --delete-branch --clean-claude           # Full cleanup
#
# This script:
...
# 6. Optionally cleans up Claude Code project data (sessions, memory)
# 7. Optionally deletes local and remote git branches
...
parse_flags() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --clean-claude) CLEAN_CLAUDE="--clean-claude" ;;
        esac
    done
}
...
if [ "$1" = "--merged" ]; then
    ...
    while IFS= read -r entry; do
        ...
        if [ "$PR_STATE" = "MERGED" ]; then
            ...
            "$SCRIPT_DIR/cleanup.sh" "$PROJECT" "$BRANCH" $DELETE_BRANCH $CLEAN_CLAUDE
            CLEANED=$((CLEANED + 1))
        else
            SKIPPED=$((SKIPPED + 1))
        fi
...
if [ -z "$PROJECT" ] || [ -z "$BRANCH" ]; then
    echo "Usage: $0 <project> <branch> [--delete-branch] [--clean-claude]"
    echo "       $0 --merged [--delete-branch] [--clean-claude]"
    exit 1
fi
...
# Step 5: Clean up Claude Code project data (if requested)
if [ "$CLEAN_CLAUDE" = "--clean-claude" ]; then
    CLAUDE_PROJECT_DIR="${HOME}/.claude/projects/$(echo "$WORKTREE_PATH" | tr '/' '-')"
    if [ -d "$CLAUDE_PROJECT_DIR" ]; then
        echo "Removing Claude Code project data: $CLAUDE_PROJECT_DIR"
        rm -rf "$CLAUDE_PROJECT_DIR" && echo "  Removed Claude project data" || echo "  Failed to remove Claude project data"
    else
        echo "No Claude Code project data found"
    fi
fi

# Step 6: Delete git branches (if requested)
